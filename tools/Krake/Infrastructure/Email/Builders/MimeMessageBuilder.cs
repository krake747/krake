...
public interface IMimeMessageBuilder :
    IResultFunctionalBuilder<Result<ErrorBase, MimeMessage>, MimeMessage, MimeMessageBuilder>,
...
public sealed class MimeMessageBuilder : IMimeMessageBuilder
{
    ...
    private readonly Errors _errors = Errors.FromCollection([]);

    public Result<ErrorBase, MimeMessage> Build() =>
        _errors.Count > 0 ? _errors : _actions.Aggregate(new MimeMessage(), (x, f) => f(x));
    ...
    private static void CreateBodyWithAttachments(IMimeMessage message, string body, EmailBodyFormat bodyFormat,
